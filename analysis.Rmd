---
title: "COVID-19 Vaccination and In-Hospital Mortality Among Immunocompromised Older Adults: A Population-Based Study From Brazil, 2024"
output:
  html_document:
    df_print: paged
    toc: true
...
### Days betwwn vaccine date and internation
...
### Alternative E - Main version, but reduces education level group
...
```{r}
filtered$rcategorical_doses <- relevel(factor(filtered$categorical_doses), ref = "unvaccinated")
filtered$reducation_level <- relevel(factor(filtered$education_level), ref = "High")


model <- glm(outcome ~ NU_IDADE_N + CS_SEXO +
               reducation_level +
               nominal_icu_admission + 
               nominal_invasive_support_ven + rcategorical_doses,
               family = binomial,
               data = filtered)
...
label_map <- c(
  "NU_IDADE_N" = "Age",
  "CS_SEXOM" = "Male",
  "reducation_levelElementary" = "Elementary Education",
  "reducation_levelMiddle" = "Middle Education",
  "reducation_levelUnknown" = "Unknown Education",
  "nominal_icu_admissionYes" = "ICU Admission",
  "nominal_invasive_support_venYes" = "Invasive Ventilation",
  "rcategorical_doses1-2" = "1-2 Doses",
  "rcategorical_doses3" = "3 Doses",
  "rcategorical_doses4+" = "4+ Doses"
)
...
coef_df$UpperCI <- exp(coef_df[, 2])
coef_df$Variable <- label_map[rownames(coef_df)]
coef_df= coef_df[row.names(coef_df) != "reducation_levelUnknown",]
